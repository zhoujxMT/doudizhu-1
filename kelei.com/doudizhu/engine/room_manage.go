...
func (r *roomManage) createRoom() *Room {
	...
	room.usersVideoIntegral = []int{0, 0, 0}
	room.setInning(1)
	room.setInnings(12)
	room.setInningRegular(6)
	room.surplusBKingCount = 4
	room.surplusSKingCount = 4
	room.surplusTwoCount = 16
	room.setCardinality(100)
	room.setMultiple(1)
	room.setLiujuMultiple(1)
	room.configRoomByGameRule()
	//	room.setGameType(GAMETYPE_DOUBLE)
	//	room.SetCardMode(CARDMODE_NOWASH)
	return &room
}
