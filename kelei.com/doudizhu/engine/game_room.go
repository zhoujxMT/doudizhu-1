...
func checkEnterRoom(userid string, matchID int, x string) *string {
	...
	if matchID == Match_JD {
		...
		res = checkEnterJD(userid, roomType)
...
out:-3房间类型不存在,-4元宝数不足,-5等级不够
	1可以进入房间
*/
func checkEnterJD(userid string, roomType int) string {
	res := "1"
	//创建玩家
...
/*
创建好友同玩房间
in:matchid|模式(0够级英雄 1开点发四)|一轮局数
...
out:任务内容,奖励积分
*/
func GetCouncilTask(args []string) *string {
	userid := args[0]
	user := UserManage.GetUser(&userid)
	if user == nil {
		return &Res_Unknown
	...
	message := "-1"
	if task != nil {
		message = fmt.Sprintf("%s,%d", task.getContent(), task.Award)
	}
	return &message
}
...
in:局数
out:1成功
push:TopChange_Push,1
*/
func UpdateInning(args []string) *string {
	userid := args[0]
	inning, _ := strconv.Atoi(args[1])
	user := UserManage.GetUser(&userid)
	room := user.getRoom()
	room.setInning(inning)
	room.gameTypeSwitch()
	pushMessageToUsers("TopChange_Push", []string{"1"}, room.getUserIDs())
	room.pushJudgment("TopChange_Push", "1")
	return &Res_Succeed
}
...
push:TopChange_Push,1
*/
func UpdateInnings(args []string) *string {
	userid := args[0]
	innings, _ := strconv.Atoi(args[1])
	user := UserManage.GetUser(&userid)
	room := user.getRoom()
	room.setInnings(innings)
	pushMessageToUsers("TopChange_Push", []string{"1"}, room.getUserIDs())
	room.pushJudgment("TopChange_Push", "1")
	return &Res_Succeed
}
...
push:TopChange_Push,1
*/
func UpdateInningRegular(args []string) *string {
	userid := args[0]
	inningRegular, _ := strconv.Atoi(args[1])
	user := UserManage.GetUser(&userid)
	room := user.getRoom()
	room.setInningRegular(inningRegular)
	room.gameTypeSwitch()
	pushMessageToUsers("TopChange_Push", []string{"1"}, room.getUserIDs())
	room.pushJudgment("TopChange_Push", "1")
	return &Res_Succeed
