...
var (
	clientManager = &ClientManager{clients: make(map[string]*Client), clientsInfo: make(map[string]*ClientInfo)}
)
...
//获取玩家信息
func (c *ClientManager) getUserInfo(uid string) *ClientInfo {
	return c.clientsInfo[uid]
}

//预保存玩家信息
func (c *ClientManager) saveUserName(uid string, userName string) {
	if c.clientsInfo[uid] == nil {
		c.clientsInfo[uid] = &ClientInfo{}
	}
	c.clientsInfo[uid].UserName = userName
}

//预保存玩家信息
func (c *ClientManager) saveUserSex(uid string, sex int) {
	if c.clientsInfo[uid] == nil {
		c.clientsInfo[uid] = &ClientInfo{}
	}
	c.clientsInfo[uid].Sex = sex
}
...
func (c *Client) getSex() int {
	return c.sex
}

func (c *Client) setSex(sex int) {
	c.sex = sex
}
...
func (c *Client) GetBaseInfo(userid string) string {
	...
	return fmt.Sprintf("%s,%s,%s,%d", userid, cl.getHeadUrl(), cl.getUserName(), cl.getSex())
}

//获取玩家基本信息(玩家不在线也可以)
func (c *Client) GetBaseInfoByUID(uid string) string {
	userInfo := clientManager.getUserInfo(uid)
	userName := ""
	sex := 1
	if userInfo != nil {
		...
		clientManager.clientsInfo[uid] = &ClientInfo{}
		userName = c.getUserNameFromDB(uid)
		sex = c.getSexFromDB(uid)
	}
	return fmt.Sprintf("%s,%s,%s,%d", uid, "", userName, sex)
}
...
//获取sex
func (c *Client) getSexFromDB(uid string) int {
	db := GetDB("game")
	sex := 1
	err := db.QueryRow("select sex from userinfo where uid=?", uid).Scan(&sex)
	logger.CheckError(err)
	clientManager.saveUserSex(uid, sex)
	return sex
}
...
func (c *Client) handleClientData(msg []byte) string {
	...
	*/
	if funcName == "Init" {
		...
		if err == nil {
			...
			userInfo := clientManager.getUserInfo(uid)
			if userInfo != nil {
				c.setUserName(userInfo.UserName)
				c.setSex(userInfo.Sex)
			}
	...
	*/
	...
	/*
		...
		in:uid,玩家名,性别
		out:1成功
		push:UpdateBaseInfo_Push,uid,玩家名,性别
	*/
	if funcName == "UpdateBaseInfo" {
		uid := p[1]
		//玩家名
		newUserName := p[2]
		...
		newSex := 1
		if len(p) > 3 {
			newSex, _ = strconv.Atoi(p[3])
			clientManager.saveUserSex(uid, newSex)
			_, err := db.Exec("update userinfo set videosex=? where uid=?", newSex, uid)
			logger.CheckError(err)
			if cl != nil {
				cl.setSex(newSex)
			}
		...
		for _, cl := range clientManager.clients {
			cl.push(c.Format(funcName+"_Push", fmt.Sprintf("%s,%s,%d", uid, newUserName, newSex)))
		}
